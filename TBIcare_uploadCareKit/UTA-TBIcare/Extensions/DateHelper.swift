...
//    static let apiDateFormatter = DateFormatter(dateFormat: Constants.DateFormat.Main)
//    static let secondaryApiDateFormatter = DateFormatter(dateFormat: Constants.DateFormat.Main)
}
...
public extension Date {
    ...
    var prev11pm: Date{
        ...
        return cal.startOfDay(for: self).adding(.hour, value: -1)
    }
    
    var coming11pm: Date{
        let cal = Calendar(identifier: .gregorian)
        return cal.startOfDay(for: self).adding(.hour, value: 23)
    }
    
    var prev6am: Date{
        let cal = Calendar(identifier: .gregorian)
        return cal.startOfDay(for: self).adding(.hour, value: 6)
    }
    
    var coming6am: Date{
        let cal = Calendar(identifier: .gregorian)
        return cal.startOfDay(for: self).adding(.hour, value: 30)
    }
}
